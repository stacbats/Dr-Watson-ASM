...
main:
    ldx #100        // 2 bytes: load 100 into X
L1: lda #90         // 2 bytes: Load 90 into A (our diamond)
    sta 1023,x   // 3 bytes: Outputs the diamond at address (1023+x)
    lda #1          // 2 bytes: Load Accumulator with 1
    sta 55295,x    // 3 bytes: Ensure the colour is white for colour Mem
    dex             // 1 byte:  decrement the X value  (243 / $f3)
